...
export function ModeSwitch({
    ...
    return (
        <div className="inline-flex overflow-hidden rounded-lg border border-[#3b494c]/30 bg-[#1c2022]">
            <button
                onClick={() => onChange("manual")}
                className={`flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold uppercase tracking-wider transition-colors ${
                    mode === "manual" ? "bg-cyan-400 text-[#101416]" : "text-slate-300 hover:text-white"
                }`}
            >
            ...
            <button
                ...
                className={`flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-semibold uppercase tracking-wider transition-colors ${
                    mode === "csv" ? "bg-emerald-400 text-[#101416]" : "text-slate-300 hover:text-white"
                }`}
            >
